...
def loader():
    ...
    print 'computing symmetric Jacobi-Hilbert matrix...'
    import pyfits
...
#    if debug: print (np.abs(f_r_c - f_rvec))**0.5
    ...
    if rf63 > d_r_c.max():
        d_ADUf63 = d_ADUrate.max()
    else: d_ADUf63 = np.interp(rf63,d_r_c,d_ADUrate)
        
    if rf4 > d_r_c.max():
        d_ADUf4 = d_ADUrate.max()
    else: d_ADUf4 = np.interp(rf4,d_r_c,d_ADUrate)
    
    tput_f63 = np.interp(rf63,f_r_c,f_ADUrate)/d_ADUf63
    tput_f4 = np.interp(rf4,f_r_c,f_ADUrate)/d_ADUf4
    tput_f63b = np.interp(rf63,f_r_c,f_EE)
    tput_f4b = np.interp(rf4,f_r_c,f_EE)
    
    FRD_4 = np.interp(rf4,f_rvec,f_ADUrate)/f_ADUrate.max()
    FRD_44 = np.interp(rf44,f_rvec,f_ADUrate)/f_ADUrate.max()
    FRD_5 = np.interp(rf5,f_rvec,f_ADUrate)/f_ADUrate.max()

    WIYN_d = np.interp(rf63,d_rvec,d_ADUrate)
    WIYN_4 = np.interp(rf4,f_rvec,f_ADUrate) / WIYN_d
    WIYN_44 = np.interp(rf44,f_rvec,f_ADUrate) / WIYN_d
    WIYN_5 = np.interp(rf5,f_rvec,f_ADUrate) / WIYN_d
    ...
    return ((metric90,metric80,tput_100,WIYN_4,WIYN_44,WIYN_5,
             FRD_4,FRD_44,FRD_5),(d_N,d_N_c,f_N,f_N_c,d_EE,f_EE,f_EE_nd))
...
def soba(nood,num_ap,dir_cut,exten,pot,mfile):
    ...
    f.write('# Output writen on: '+datetime.now().isoformat(' ')+'\n'
            ...
            +'# {:10}= '.format('wiyn4')+'fiber within f/4 / direct within f/6.3 (uncorrected)\n'
            +'# {:10}= '.format('wiyn44')+'fiber within f/4.4 / direct within f/6.3 (uncorrected)\n'
            +'# {:10}= '.format('wiyn5')+'fiber within f/5 / direct within f/6.3 (uncorrected)\n'
            +'# {:10}= '.format('FRD4')+'fiber f/4 / total fiber\n'
            +'# {:10}= '.format('FRD44')+'fiber f/4.4 / total fiber\n'
            +'# {:10}= '.format('FRD5')+'fiber f/5 / total fiber\n'
            +'#\n'
            +str('#{0:>10}{1:>10}{2:>9}{3:>9}{4:>9}{5:>9}{6:>9}{7:>9}'
                 +'{8:>9}{9:>9}{10:>9}{11:>9}\n')\
                .format('Fiber_pos','Out_pos','filt','N90','N80','tput','WIYN4',\
                            'WIYN44','WIYN5','FRD4','FRD44','FRD5')
            +str('#{0:>10}{1:>10}{2:>9}{3:>9}{4:>9}{5:>9}{6:>9}{7:>9}'
                 +'{8:>9}{9:>9}{10:>9}{11:>9}\n')\
    ...
    for fiber_pos in sorted(nood.iterkeys(), key=lambda k: nood[k]['outpos']):

        print 'Fiber ('+str(fiber_pos)+')'
        try: outpos = nood[fiber_pos]['outpos']
        except KeyError: outpos = 'NA'
        print 'Slit position: {}'.format(outpos)
        ...
        for filt in nood[fiber_pos]['data'].keys():
            ...
            hdu.header.update('WIYN',metric[3])
            hdulist.append(hdu)
...
def webit(name):
    ...
    ht.write('<h4>All-fiber grid</h4>\n'
             +"if the jpg links don't work try the pdf<br>\n"
    ...
    old_m = open('/d/www/eigenbrot/PAK/index.html')
    lines = old_m.readlines()
    old_m.close()

    lines[3] = '<h2>Last updated '+datetime.now().isoformat(' ')+'</h2></h1>\n'
    m = open('/d/www/eigenbrot/PAK/index.html','wb')
...
#            ax.plot(plot_data[0],plot_data[4],'r:',lw=0.4)
            ...
            ax.text(9,0.7,'({0})\n{1:4.3f}'.format(fiber_pos,WIYN_metric))
            ax.set_xlim(2,30)
            ax.set_ylim(0,1.1)
            ax.set_xscale('log')
