...
function bc_continuum_allZ, model, restwl, flux, err, vdisp, emmaskw=emmaskw, $
...
parinfo = replicate({value:1.D, fixed:0, limited:[0,0], tied:'', $
                    limits:[0.0,0], step:0}, nmodels * 2 + 1)

parinfo[0].limited = [1,1]
parinfo[0].limits = [0,20]
parinfo[1:nmodels].value = fix(4)
parinfo[1:nmodels].fixed = 0
parinfo[1:nmodels].limited = [1,1]
parinfo[1:nmodels].limits = [0,5]
parinfo[1:nmodels].step = 1
...
custom_lib = dblarr(npix, nmodels, nmetals)
for ii = 0, nmodels - 1 do begin
   for jj = 0, nmetals - 1 do begin
      cflux = gconv(model.flux[*,ii,jj], sigma_pix) ; convolve with gaussian
      custom_lib[*,ii,jj] = interpol(cflux, model.wave, restwl)
   endfor
endfor
...
fitcoefs = mpfitfun('bc_mcombine_allZ', fitwave, fitflux, fiterr, $
                    parinfo = parinfo, $
                    functargs = {biglib: fitlib, savestep: savestep}, $
                    perror=perror, niter=niter, status=status, $
                    errmsg=errmsg, maxiter = 50000, xtol=1d-17, ftol=1d-10, /NAN)
...
yfit = bc_mcombine_allZ(restwl, fitcoefs, biglib=custom_lib)
...
final_norm = make_array(nmodels)

for nn = 0, nmodels - 1 do begin
   final_norm[nn] = model.norm[nn,fitcoefs[nn+1]]
endfor

SNR = mean(flux[lightidx]/err[lightidx])

MMWA = total(model.age/1e9*fitcoefs[nmodels + 1:*]*light_factor/final_norm) $
          / total(fitcoefs[nmodels + 1:*]*light_factor/final_norm)
...
light_weight = mean(model.flux[lightidx,*] * rebin(redd,n_elements(lightidx),$
                    ...
                    dimension=1) * fitcoefs[nmodels + 1:*] * light_factor
...
coefs = {tauv: fitcoefs[0], tauv_err: perror[0], $
         ...
         Z: fitcoefs[1:nmodels], Z_err: perror[1:nmodels], $
         model_age: model.age, chisq: chisq, $
         redchi: redchi, bluechi: bluechi, hkchi: hkchi, $
         MMWA: MMWA, MLWA: MLWA, SNR: SNR, final_norm: final_norm}
...
for i=0, nmodels - 1 do begin
   yi = coefs.light_frac[i] * custom_lib[*,i,coefs.Z[i]] * $
...
for i = 0, n_elements(coefs.light_frac) - 1 do begin
   xyouts, 0.8, 0.55 - i*0.02, string(model.age[i]/1e9, ': ', coefs.light_frac[i], ', ', coefs.Z[i],$
