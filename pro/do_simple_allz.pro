...
pro do_simple_allZ, datafile, errorfile, output, location=location, $
...
FOR k=0, numages - 1 DO BEGIN
   ...
   Zarr[k] = string(agearr[k],' Z',format='(F6.3,A4)')
ENDFOR

t3d, /reset;, translate=[-1,-1,0], rotate=[0,0,180]
fmt = '(I11,'+string(numages*2 + 2)+'E13.3,6F12.3)'
...
printf, lun, '# Fiber Num',colarr,Zarr,'MMWA [Gyr]','MLWA [Gyr]',$
        'Tau_V','S/N','Chisq','redChi','blueChi','HKChi',$
        format='(A-11,'+string(numages*2 + 2)+'A13,6A12)'
printf, lun, '#'
...
outputarray = {TAUV: 0.0D, TAUV_ERR: 0.0D, LIGHT_FRAC: dblarr(10),$
               LIGHT_FRAC_ERR: dblarr(10), Z: dblarr(10), Z_err: dblarr(10), $
               MODEL_AGE: dblarr(10), $
               CHISQ: 0.0D, REDCHI: 0.0D, BLUECHI: 0.0D, HKCHI: 0.0D, $
               MMWA: 0.0D, MLWA: 0.0D, SNR: 0.0D, final_norm: fltarr(10)}
...
; fit continuum
   print, vd
   coef = bc_continuum_allZ(m, wave, flux, err, vd, $
                            plotlabel=plotlabel, $
                            bluefit=bluefit, $
                            yfit=continuum, $
...
;mwrfits, s, 'NGC_test.fits', /create
   ...
   printf, lun, i+1, coef.light_frac/coef.final_norm, coef.Z, coef.MMWA, $
           coef.MLWA, coef.tauv, coef.SNR, coef.chisq, coef.redchi, $
           coef.bluechi, coef.hkchi, format=fmt
