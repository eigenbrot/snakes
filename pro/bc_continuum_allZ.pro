...
for nn = 0, n_models - 1 do begin
   final_norm[nn] = model.norm[nn,fitcoefs[nn+1]]
endfor

SNR = mean(flux[lightidx]/err[lightidx])

MMWA = total(model.age/1e9*fitcoefs[1:*]*1./final_norm) $
          / total(fitcoefs[1:*]*1./final_norm)
...
coefs = {tauv: fitcoefs[0], tauv_err: perror[0], $
         ...
         light_frac: fitcoefs[nmodels + 1:*], light_frac_err: perror[nmodels + 1:*], $
...
for i=0, nmodels - 1 do begin
   yi = fitcoefs[i+nmodels+1] * custom_lib[*,i,coefs.Z[i]] * $
