    ...
    for i in range(numfibers):
        ...
        for j in range(coefs['LIGHT_FRAC'].size):
            ...
            yi = coefs['LIGHT_FRAC'][j] * custom_lib[j,:] *\
                 np.exp(-1 * coefs['TAUV'][j]*(restwl/5500.)**(-0.7))
        ...
        fig.text(0.35, 0.87, 'MMWA = {:8.2f}'.format(coefs['MMWA']),
                 fontsize=fs)
        fig.text(0.35, 0.85, 'MLWT = {:8.2f}'.format(coefs['MLWT']),
                 fontsize=fs)
        fig.text(0.35, 0.83, 'MMWT = {:8.2f}'.format(coefs['MMWT']),
                 fontsize=fs)
        ...
        for t in range(coefs['TAUV'].size):
            fig.text(0.75, 0.89 - 0.011*t, '{:>7.3f}:  {:5.3f}'.format(m['AGE'][t,0]/1e9,coefs['TAUV'][t]),
                     fontsize=7)
